...
-------------------- basic set up configuring command  in python ---------------------------
htmlpreview.github.io/?
install vusual basic -> intall python related plugin(python for vscode and python from microsoft to improve code writing) -> 
----------------cmd for use ------
-> type nul > abc.txt       # to create a file using cmd
-> rename abc.txt abc.py
-> notepad++ abc.py
-> abc.py
-> pythong abc.py
-> idle
-> python --version
-> py --version
-> where python   ## find the location of the 
-> set PY_PYTHON 2
   set PY_PYTHON=2
-> code .      // in cmd to lunch visual studio code in current folder
-> py -2 myapp.py    // to lunch python 2 to execute my code at run time
-> py -3 myapp.py    // to lunch python 3 to execute my code at run time
---- command used in VSC(visual studio code)
> alt + shift + a #to comment the lines 
> ctrl+shift +p ... configure task  // create task.json
>> ctrl+shift +p ... run task // provide option which task to run
===================================================================================
#https://code.visualstudio.com/docs/python/tutorial-flask
#The default Python implementation, CPython, is actually written in the C programming language.
python -> to lunch python cli in cmd
exit() -> exit from python cli
list = [1,2,3]
tuple=(1,3,4)
set={1,2,4}
dict={'name':'ravi','dept':'cse'}
print(dir(sys))
...
yield                               # return the value and resume from the same place in another call next(obj)
...
#How can we add function attribute after creating the object or after creating class:
  ##for method to get add to the class but not to only instance level  of the class:
...
spread operator list /set:  *passingargs   # if we use it with dict.. then only key of the dict will get spread ...
...
# Q) How can we create empty body function and call it?
   #hint: use pass keyword  and same goes for class also
...
# Q) what is difference between input() and raw_input() function  #can be understand with python 2 perspective.
...
Ans: PYTHONPATH: this variable value only loaded once at the time of starting the application.
  ...
  Example to import my custom module in cli: 
     ...
     #h.main()
Q) what is differnece between yield and return statement?
...
>>> s.count('i')
1 """
# what is use of f'string in python?
...
# prolem with the second way of import is:
  ...
  Solution: We will create a alias of the imported function or if two module is having same name.. then will create a alias Name
   ...
   #NOTE: import along with creating a module object and import it. it execute the module code also and all above way of importing
          #module execute the module file global code before imported.
